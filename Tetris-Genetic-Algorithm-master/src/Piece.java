...
public class Piece {
    ...
    public int endOfPiece(String str) {
        int subVal = 1;
        if (str.indexOf("-")!=-1){
            subVal=2;
    ...
    public boolean validMove(String shape, int[] currentPos, boolean[][] grid){
        int index = shape.indexOf("o");

        
        while (index!=-1){
            if (currentPos[1]+index/4 >= numBoxY || grid[currentPos[0]+index%4][currentPos[1]+index/4]){
                return false;
            }
